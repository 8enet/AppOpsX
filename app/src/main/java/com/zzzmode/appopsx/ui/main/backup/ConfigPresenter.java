...
class ConfigPresenter {
    ...
    void restoreOps(final RestoreModel model){
        ...
        Observable.fromIterable(model.preAppInfos).observeOn(Schedulers.io()).flatMap(new Function<PreAppInfo, ObservableSource<OpsResult>>() {
            @Override
            public ObservableSource<OpsResult> apply(@NonNull PreAppInfo appInfo) throws Exception {
                return Helper.setModes(context,appInfo.getPackageName(),AppOpsManager.MODE_IGNORED,appInfo.getOps());
            }
