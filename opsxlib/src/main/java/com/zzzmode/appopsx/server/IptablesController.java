...
/**
 *
 iptables -N appopsx_wifi_reject_list
 iptables -N appopsx_mobile_reject_list
 iptables -N appopsx_bw

 iptables -A INPUT -j appopsx_bw
 iptables -A OUTPUT -j appopsx_bw

 iptables -A appopsx_bw -o wlan+ -j appopsx_wifi_reject_list
 iptables -A appopsx_bw -o tiwlan+ -j appopsx_wifi_reject_list

 iptables -A appopsx_bw -o rmnet+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o rmnet_data+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o pdp+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o ppp+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o uwbr+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o wimax+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o vsnet+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o ccmni+ -j appopsx_mobile_reject_list
 iptables -A appopsx_bw -o eth_x+ -j appopsx_mobile_reject_list

 iptables -A appopsx_wifi_reject_list -m owner --uid-owner 10255 -j REJECT --reject-with icmp-port-unreachable

 iptables -D appopsx_wifi_reject_list -m owner --uid-owner 10255 -j REJECT --reject-with icmp-port-unreachable
...
class IptablesController {
    ...
    private boolean check(String rejectList,int uid){
        try {
            Shell.Result result = runIptablesCmd(" -C "+String.format(Locale.ENGLISH,rejectList+RULE_REJECT,uid));
